...
async function fillInputResilient(locator, value, { timeout = 15_000 } = {}) {
  ...
  // Primary: click + type with human-like random delay to avoid CAPTCHA triggers.
  // Avoid fill() as primary — it sets the value instantly which Google detects.
  try {
  ...
  try {
    const delay = Math.floor(40 + Math.random() * 80); // 40-120ms per keystroke
    await locator.type(value, { delay, timeout });
  } catch {
    // If type() fails, fall back to fill()
    try {
  ...
  // Final fallback for React/Vue-controlled inputs
  try {
    await locator.click({ timeout: 5_000 });
  } catch {
    /* noop */
