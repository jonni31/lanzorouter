...
// server-side cache snapshot. Used by the /api/usage/aggregate route (bulk view)
// and available for a future background refresh loop. Extracted so the aggregate
// path shares one implementation of the refresh→fetch→cache flow WITHOUT touching
// the per-connection live route (src/app/api/usage/[connectionId]/route.js).
 ...
 * Mirrors the logic in [connectionId]/route.js so the aggregate path stays
 * independent of the live route.
 ...
 * to the server-side quota cache keyed by the connection's (split) provider id.
 * Does not throw for expected states.
 *
...
export async function resolveUsageForConnection(connectionId, opts = {}) {
  ...
  if (isOAuth) {
    try {
      ...
      return { error: `Credential refresh failed: ${refreshError.message}` };
    }
  }
