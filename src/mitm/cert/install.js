...
const ROOT_CA_CN = "Lanzo Router MITM Root CA";
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "Lanzo Router MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
async function updateNssDatabases(certPath, action = 'add') {
  const certName = "Lanzo Router MITM Root CA";
