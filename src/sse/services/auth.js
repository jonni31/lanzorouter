  ...
  if (resetsAtMs && resetsAtMs > Date.now()) {
    ...
    if (shouldFallback && provider) {
      try {
        ...
        const _isQuotaType = status === 402 || status === 429 || newBackoffLevel !== undefined;
        if (_s.providerHealthQuotaCooldownEnabled?.[provider] && _isQuotaType) {
          const _min = _s.providerHealthQuotaCooldownMinutes?.[provider] ?? 30;
          cooldownMs = _min * 60 * 1000;
          newBackoffLevel = 0;
        }
