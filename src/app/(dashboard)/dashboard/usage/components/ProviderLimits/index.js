...
export default function ProviderLimits() {
  ...
  // Fetch server-side aggregate quota for bulk display mode
  ...
  const refreshAggregate = useCallback(
    async (connectionIds, provider) => {
      // In bulk mode, refresh via server-side aggregate (covers ALL accounts)
      if (displayMode === "bulk" && provider) {
        await fetchServerAggregate(provider, true);
        setLastUpdated(new Date());
        return;
      }
    ...
    [fetchQuota, displayMode, fetchServerAggregate],
  );
  ...
  const refreshAll = useCallback(async () => {
    ...
    try {
      ...
      // Also refresh server-side aggregate if in bulk mode
      if (displayMode === "bulk") {
        const providers = providerFilter !== "all"
          ? [providerFilter]
          : providerOptions.map((opt) => opt.value || opt).filter((v) => v !== "all");
        providers.forEach((provider) => fetchServerAggregate(provider, true));
      }
  ...
  }, [refreshingAll, fetchConnections, fetchQuota, page, displayMode, providerFilter, providerOptions, fetchServerAggregate]);
  ...
  // When in bulk mode, fetch server-side aggregate for the active provider filter
  useEffect(() => {
    if (displayMode !== "bulk") return;
    const providers = providerFilter !== "all"
      ? [providerFilter]
      : providerOptions.map((opt) => opt.value || opt).filter((v) => v !== "all");
    providers.forEach((provider) => fetchServerAggregate(provider));
