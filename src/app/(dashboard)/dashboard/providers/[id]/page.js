...
import { CONNECTION_STATUS_FILTERS, filterConnectionByStatus } from "@/shared/utils/connectionStatus";
                ...
                <div className="mb-3 flex items-center gap-2 border-b border-black/[0.03] pb-2 dark:border-white/[0.03]">
                  ...
                  <div className="flex flex-1 flex-wrap items-center justify-end gap-1">
                    {CONNECTION_STATUS_FILTERS.map((filter) => {
                      const count = connectionStatusCounts[filter.id] ?? 0;
                      const isActiveFilter = connectionStatusFilter === filter.id;
                      return (
                        <button
                          key={filter.id}
                          type="button"
                          onClick={() => setConnectionStatusFilter(filter.id)}
                          className={`rounded-full px-2 py-0.5 text-xs transition-colors ${isActiveFilter ? "bg-primary text-white" : "bg-black/[0.04] text-text-muted hover:text-primary dark:bg-white/[0.06]"}`}
                        >
                          {filter.label} {count}
                        </button>
                      );
