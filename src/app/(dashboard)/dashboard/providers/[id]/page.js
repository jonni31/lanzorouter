...
export default function ProviderDetailPage() {
  ...
  const handleBulkDeleteByEmail = (emailFilter) => {
    const label = emailFilter === "gsuite" ? "GSuite (non-Gmail)" : "Gmail (@gmail.com)";
    setConfirmState({
      title: `Delete ${label}`,
      message: `Delete ALL ${label} accounts for this provider? This cannot be undone.`,
      onConfirm: async () => {
        setConfirmState(null);
        try {
          const res = await fetch("/api/providers/bulk-delete", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ provider: providerInfo?.id || providerId, emailFilter }),
          });
          if (res.ok) {
            setSelectedConnectionIds([]);
            await fetchConnections();
                ...
                {selectedConnections.length > 0 && (
                  <>
                    ...
                    <Button
                      ...
                      variant="secondary"
                      icon="domain"
                      onClick={() => handleBulkDeleteByEmail("gsuite")}
                    >
                      Delete GSuite
                    </Button>
                    <Button
                      ...
                      variant="secondary"
                      icon="mail"
                      onClick={() => handleBulkDeleteByEmail("gmail")}
                    >
                      Delete Gmail
                    </Button>
                  </>
