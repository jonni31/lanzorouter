...
import { getAntigravityEnvelopeUserAgent } from "../services/antigravityIdentity.js";
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials, stream = true, sessionId = null) {
    // Scrub proxy-tracing + Chromium/Stainless fingerprint headers so the
    // upstream request matches the native Antigravity desktop-client fingerprint.
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    // Obfuscate third-party coding-tool names (zero-width joiner) so Google's
    // account-scanner can't grep them; set per-account envelope UA (antigravity/jetski).
    return obfuscateRequestBody({
      ...body,
      project: projectId,
      model: model,
